...
class PGBaseModel {
    ...
    static getTableName(fieldValues) {
        return this.table;
    }
    ...
    static async findAllBy(fieldValues, operator = 'AND', limit, params = '*') {
        ...
        var query = `SELECT ${params} FROM ${this.getTableName(fieldValues)} WHERE ${builtQuery.query} ${limit ? 'LIMIT ' + limit : ''}`;
    
        return this._convertToModel(await pg.query(query, builtQuery.values));
    }
 
    static async findAll(limit, params = '*') {
        return this._convertToModel(await pg.query(`SELECT ${params} FROM ${this.getTableName()} ${limit ? 'LIMIT ' + limit : ''}` ));
    }
    ...
    static async deleteAllBy(fieldValues, operator = 'AND', limit) {
        var builtQuery = this._builtQuery(fieldValues, operator);
        var query = `DELETE FROM ${this.getTableName(fieldValues)} WHERE ${builtQuery.query} ${limit ? 'LIMIT ' + limit : ''} RETURNING *;`;

        return this._convertToModel(await pg.query(query, builtQuery.values));
    }

    static async deleteAll() {
        var query = `DELETE FROM ${this.getTableName()} RETURNING *;`;
    ...
    static async create(model, returnModel = true) {
        ...
        var query = `INSERT INTO ${this.getTableName(model)} (${keys.toString()}) VALUES(`;
    ...
    static async updateAllBy(fieldValues, model, operator = 'AND', returnModel = true, limit) {
        ...
        var query = `UPDATE ${this.getTableName(fieldValues)} SET `;
    ...
    static async updateAll(model) {
        ...
        var query = `UPDATE ${this.getTableName(model)} SET `;
